...
public class PalindromeFill {
    ...
    //只能在字符串前面（或者后面）添加字符，使用最少的字符构造回文字符串
    //查找以首字符（或者尾字符）为端点的最长回文子串，也就是前缀（或者后缀）最长回文子串，剩余的字符即为需要填充的字符
    //KMP算法：将原字符串的反转字符串拼接到原字符串的后面得到新的字符串，查找新字符串最长相等的前缀与后缀，并且长度不超过原字符串
    //最长前缀即为以首字符为端点的最长回文子串
